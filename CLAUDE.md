...
Inside the container, an inline bash entrypoint starts: dbus-daemon (socket in shared dir), KWin `--virtual --xwayland --width/--height` at the per-session resolved size (see Virtual screen below), AT-SPI, PipeWire, WirePlumber. The host connects to the container's D-Bus via the shared socket. Apps are launched by writing commands to bwrap's stdin.
...
- **Virtual screen**: `VIRTUAL_SCREEN_WIDTH`/`VIRTUAL_SCREEN_HEIGHT` (3840x2160 since 2026-06-09, matching the user's primary 4K monitor) are only the compiled-in *defaults*. Precedence: `session_start` optional `width`/`height` params > `--width`/`--height` CLI flags (set via `args` in `~/.mcp.json`) > these constants. The `--no-override` CLI flag pins the CLI/compiled size — tool params are then silently ignored (logged to stderr), so agents can't resize. All dims are validated to `MIN_SCREEN_DIM..=MAX_SCREEN_DIM` (240..=8192; max matches the viewer's PipeWire format-pod ceiling). The resolved size is threaded through the bash entrypoint's `kwin_wayland --width/--height`, stored in `Session` (`screen_width`/`screen_height`, reported in every session_start result), and passed to the viewer as argv `[session_dir, width, height]` — `kwin-viewer.rs` uses it for `map_window_to_virtual` input mapping and the PipeWire preferred `VideoSize` (defaults to 3840x2160 if invoked manually without the args). Size changes take effect at the next `session_stop`+`session_start`; no server restart needed.
...
`session_start` spawns an isolated KWin Wayland session (per-session size, XWayland, own D-Bus) and is idempotent; optional `width`/`height` params override the server default unless `--no-override` locked it. All other tools require an active session. `session_stop` kills the process group. Input tools (`mouse_*`, `keyboard_*`) use window-relative coordinates — window position is added internally via `active_window_info()` KWin scripting. Screenshots go through `org.kde.KWin.ScreenShot2` D-Bus interface, returning raw ARGB32 converted to PNG. `accessibility_tree` traverses AT-SPI2 with configurable depth/filters. `find_ui_elements` searches by name/role — automatically uses CDP DOM queries for Chromium/Electron apps, AT-SPI for native apps. `launch_app` auto-detects Chromium apps by injecting `--remote-debugging-port` and attempting CDP connection. Chromium/Chrome in the container transparently read the host's kwallet data (cookies decrypt, saved passwords available) via the emulator — no prompts.
...
- Chromium's AT-SPI tree needs two switches or `accessibility_tree`/`find_ui_elements` see an app node with zero children: `org.a11y.Status.IsEnabled=true` on the container session bus (set by `session_start`; the ATK bridge is dormant while it's false) and `--force-renderer-accessibility` (auto-appended by `launch_app`; without it web content never appears, only the browser frame). This is the only a11y path for Google Chrome/Edge, which block CDP on the default profile.
- The `--virtual` backend already composites on the real GPU: the container kwin holds fds on `/dev/dri/renderD128` with EGL/GBM (verified 2026-06-09). Only the output side is virtual — refresh is hardcoded 60Hz (no flag; would need a kwin source patch) and there is no real CRTC/scanout. `kwin_wayland --drm` does NOT work nested: it hangs with `amdgpu_query_info(ACCEL_WORKING) failed (-13)` because the host compositor holds DRM master and a second master is impossible while the desktop session runs.
